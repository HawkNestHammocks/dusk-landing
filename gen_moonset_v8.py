#!/usr/bin/env python3
# v8: G5 = LOCKED MOONSET label — "NATURAL SLEEP SUPPLEMENT" descriptor. + standalone logo mark for site header.
import os
...
def mmark(w, gold="#cbb37e"):
    return f'''<svg viewBox="0 0 220 200" style="width:{w}px">
  <circle cx="100" cy="110" r="82" fill="none" stroke="{gold}" stroke-width="2"/>
...
L["moonset-g5"] = f'''<!DOCTYPE html><html><head><meta charset="utf-8">{F}<style>
...
.what{{margin-top:22px;font-weight:700;font-size:30px;letter-spacing:5.5px;padding-left:5px;color:#e0c88f}}
...
<div class="what">NATURAL SLEEP SUPPLEMENT</div>
...
# standalone logo mark (transparent bg) for the site header
L["moonset-mark"] = f'''<!DOCTYPE html><html><head><meta charset="utf-8"><style>
*{{margin:0}}body{{background:transparent}}.w{{width:440px;height:400px;display:flex;align-items:center;justify-content:center}}
</style></head><body><div class="w">{mmark(400)}</div></body></html>'''
